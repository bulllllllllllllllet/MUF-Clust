...
* nuclei_mask.png：二值分割掩码展示（含分水岭分割结果）。
* nuclei_edge_overlay.png：分割边缘叠加到原始 DAPI 图，检查过/欠分割。
* nuclei_distance_map.png：距离变换/分水岭种子可视化。
* 指标阈值：
...
| 模式    | 描述                            | 适用场景         |
| ----- | ----------------------------- | ------------ |
| 固定半径法 | 区域半径 = 核直径的 1.5-2 倍           | 简单、速度快       |
| 细胞级分割 | 调用 Cellpose / Mesmer 获取真实细胞轮廓 | 对膜标记/胞质信号更准确 |
...
* 模式一致性：在 Cellpose/Mesmer 模式下，边界与膜/胞质信号对齐误差（平均偏移 ≤ 3 px）。
